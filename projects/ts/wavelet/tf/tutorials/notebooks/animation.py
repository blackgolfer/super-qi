...
def animate_discrete_convolution(x, h, y, k, kappa, interval=100):
    ...
    def animate(kappa_i):
        ...
        dot.set_data(kappa_i, yf[-k[0] + kappa_i])
    ...
    stem_x = ax[0].stem(k, x(k), linefmt='C0-', markerfmt='C0o',
                        basefmt=' ', label=r'$x[\kappa]$',use_line_collection=False)
    stem_h = ax[0].stem(k, h(kappa[0]-k), linefmt='C1-',
                        markerfmt='C1o', basefmt=' ', label=r'$h(k - \kappa)$',use_line_collection=False)
    ax[0].set_xlabel(r'$\kappa$')
    ax[0].legend(loc='upper right')
    ax[0].grid()

    yf = y(k)
    ax[1].stem(k, yf, linefmt='C2-', markerfmt='C2o', basefmt=' ',
               label=r'$y[k]$')
    dot, = ax[1].plot([], 'ro')
    ax[1].set_xlabel(r'$k$')
