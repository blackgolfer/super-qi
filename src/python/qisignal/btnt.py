...
class btn:
    ...
    """
    def __init__(self,content:C=None,left=None,right=None,tag:T=None):
...
# 产生`level`层的二叉树
def makebtnt(level:int,content:C=None,lp:Callable[[C],C]=None,rp:Callable[[C],C]=None):
    """
    Generate btn tree
...
def btn2branch(self, level, block):
    if level>0:
        if (block%2)==0:
            if self.left==None:
                self.left=btn(None,None,None,None)
            self = btn2branch(self.left, level-1, block/2)
        else:
            if self.right==None:
                self.right=btn(None,None,None,None)
